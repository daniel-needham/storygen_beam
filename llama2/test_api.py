...
## todo delete  
def call_api2(payload):
    # Define the API endpoint
    api_endpoint = "https://as6xb.apps.beam.cloud/test"
    ...
    if stream:
      ...
      if response.status_code == 200:
          # Iterate over the streaming response content
          for chunk in response.iter_conte4nt(chunk_size=None):
              # Process the chunk (assuming it's JSON)
              chunk = chunk.decode("utf-8").rstrip("\0")
              data = json.loads(chunk)
              print(data['text'][0])  # Do whatever processing needed with the data
...
call_api2({"prompt": prompt, "stream": False, "sampling_params": sampling_params})
